...
@pytest.mark.parametrize(
    ("left_node_id", "right_node_id", "distance"),
    (
...
@pytest.mark.parametrize(
    ("left_node_id", "right_node_id", "log_distance"),
    (
...
def test_remove(routing_table, center_node_id):
    ...
    routing_table.remove(
        node_id_4
    )  # remove from replacement cache, shouldn't appear again
...
def test_iter_all_random(routing_table, center_node_id):
    ...
    assert (
        len(nodes_in_iteration_order) == table_length == len(nodes_in_insertion_order)
    )
...
def test_get_nodes_at_log_distance(routing_table, center_node_id, bucket_size):
    nodes = tuple(
        NodeIDFactory.at_log_distance(center_node_id, 200) for _ in range(bucket_size)
    )
    farther_nodes = tuple(
