...
from ddht.tools.v5_strategies import enr_seq_st, id_nonce_st, magic_st, nonce_st, tag_st
...
def test_auth_tag_packet_encoding_decoding(tag, auth_tag, encrypted_message_size):
    ...
    original_packet = AuthTagPacketFactory(
        tag=tag, auth_tag=auth_tag, encrypted_message=encrypted_message
    )
...
def test_oversize_auth_tag_packet_encoding():
    packet = AuthTagPacketFactory(
        encrypted_message=b"\x00"
        * (DISCOVERY_MAX_PACKET_SIZE - (1 + TAG_SIZE) - NONCE_SIZE + 1)
    )
...
@given(
    ...
    encrypted_message_size=st.integers(
        ...
        max_value=DISCOVERY_MAX_PACKET_SIZE
        - TAG_SIZE
        - sum(
            (
                2,  # rlp list prefix
...
def test_auth_header_packet_encoding_decoding(
    tag,
    auth_tag,
    id_nonce,
...
def test_oversize_auth_header_packet_encoding():
    ...
    packet = AuthHeaderPacketFactory(
        auth_header=auth_header, encrypted_message=encrypted_message
    )
    with pytest.raises(ValidationError):
        packet.to_wire_bytes()
...
@pytest.mark.parametrize(
    ...
    (
        ...
        b"\x00" * TAG_SIZE
        + rlp.encode(
            (b"\x00" * NONCE_SIZE, b"\x00" * ID_NONCE_SIZE, AUTH_SCHEME_NAME, b"")
        ),
        ...
        b"\x00" * TAG_SIZE
        + rlp.encode(
            (b"\x00" * NONCE_SIZE, b"\x00" * ID_NONCE_SIZE, b"no-gcm", b"", b"")
        ),
        # auth header with tag being a list
        b"\x00" * TAG_SIZE
        + rlp.encode(
            ([b"\x00"] * NONCE_SIZE, b"\x00" * ID_NONCE_SIZE, b"gcm", b"", b"")
        ),
        ...
        b"\x00" * TAG_SIZE
        + rlp.encode((b"\x00" * NONCE_SIZE, b"\x00" * ID_NONCE_SIZE, b"gcm", b"", b""))
        + b"\x00" * 2000,
...
@given(magic=magic_st, token=nonce_st, id_nonce=id_nonce_st, enr_seq=enr_seq_st)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        b"\x00" * (MAGIC_SIZE - 1)
        + rlp.encode((b"\x00" * NONCE_SIZE, b"", 0)),  # too short
...
@pytest.mark.parametrize(
    "packet",
    (WhoAreYouPacketFactory(), AuthTagPacketFactory(), AuthHeaderPacketFactory()),
)
    ...
    [
        ...
        [
            WhoAreYouPacket,
            {
                ...
                "token": decode_hex("0x020202020202020202020202"),
                "id_nonce": decode_hex(
        ...
        [
            ...
            {
                "tag": decode_hex(
                    "93a7400fa0d6a694ebc24d5cf570f65d04215b6ac00757875e3f3a5f42107903"
                ),
                "auth_header": AuthHeader(
                    auth_tag=decode_hex("0x27b5af763c446acd2749fe8e"),
                    id_nonce=decode_hex(
                        "e551b1c44264ab92bc0b3c9b26293e1ba4fed9128f3c3645301e8e119f179c65"
                    ),
                    auth_scheme_name=AUTH_SCHEME_NAME,
                    ephemeral_public_key=decode_hex(
                        "b35608c01ee67edff2cffa424b219940a81cf2fb9b66068b1cf96862a17d353e"
                        "22524fbdcdebc609f85cbd58ebe7a872b01e24a3829b97dd5875e8ffbc4eea81"
                    ),
                    encrypted_auth_response=decode_hex(
                        "570fbf23885c674867ab00320294a41732891457969a0f14d11c995668858b2a"
                        "d731aa7836888020e2ccc6e0e5776d0d4bc4439161798565a4159aa8620992fb"
                        "51dcb275c4f755c8b8030c82918898f1ac387f606852"
                    ),
                ),
                "encrypted_message": decode_hex(
            ...
            decode_hex(
                "93a7400fa0d6a694ebc24d5cf570f65d04215b6ac00757875e3f3a5f42107903"
                "f8cc8c27b5af763c446acd2749fe8ea0e551b1c44264ab92bc0b3c9b26293e1b"
                "a4fed9128f3c3645301e8e119f179c658367636db840b35608c01ee67edff2cf"
                "fa424b219940a81cf2fb9b66068b1cf96862a17d353e22524fbdcdebc609f85c"
                "bd58ebe7a872b01e24a3829b97dd5875e8ffbc4eea81b856570fbf23885c6748"
                "67ab00320294a41732891457969a0f14d11c995668858b2ad731aa7836888020"
                "e2ccc6e0e5776d0d4bc4439161798565a4159aa8620992fb51dcb275c4f755c8"
                "b8030c82918898f1ac387f606852a5d12a2d94b8ccb3ba55558229867dc13bfa"
                "3648",
            ),
        ],
    ],
